  ...
  return (
    <header className="border-b border-zinc-300 bg-white shadow-sm">
      <div className="mx-auto max-w-6xl px-4">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-zinc-200 bg-yellow-100 px-2 py-2 text-xs">
        ...
        <div className="flex flex-col items-center gap-3 py-5">
          <div className="flex items-center gap-3">
            <h1 className="text-5xl font-bold text-green-700 md:text-6xl">Jornal</h1>
            <Image src="/api/assets/logo" alt="Logo Jornal Marabá" width={108} height={60} className="h-12 w-auto md:h-14" />
            <h1 className="text-5xl font-bold text-green-700 md:text-6xl">Marabá</h1>
          </div>

          <nav className="flex flex-wrap items-center justify-center gap-x-2 gap-y-1 border-y border-zinc-300 py-2 text-sm font-semibold">
            {NAV_EDITORIAS.map((item) => (
              <span key={item} className="after:mx-2 after:content-['-'] last:after:content-['']">
                <Link href={item === "Colunistas" ? "/colunistas/da-redacao" : "#"}>{item}</Link>
              </span>
            ))}
          </nav>
        </div>
